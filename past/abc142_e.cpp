...
int main() {
  ...
  for (ll i = 1; i < M + 1; i++) {
    for (ll n = 0; n < ALL; n++) {
      ...
      // cout << b2 << " ";
      // bitset<4> b1((n | C[i]));
      // cout << C[i] << " : " << b1 << " : " << n << " : " << C[i] << " : " << cost[i][(n | C[i])] << " : " << cost[i - 1][n] << " " << A[i] << endl;
      cost[i][(n | C[i])] = min(cost[i][(n | C[i])], cost[i - 1][n] + A[i]);
      // cost[i][(n | S[i])] = min(cost[i][(n | S[i])], cost[i - 1][n] + C[i]);
    }
  }
  // cout << cost[0].size() << endl;
  // rep(i, cost.size()) {
  //   rep(j, cost[i].size()) {
  //     cout << cost[i][j] << " ";
  //   }
  ...
  if (cost[M][ALL - 1] == 1e18) {
    cout << -1 << endl;
  } else {
    cout << cost[M][ALL - 1] << endl;
  }
